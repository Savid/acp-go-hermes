...
docs-audit:
	...
	@for flag in -path -home -scratch-dir -provider-auth-root -provider-auth-direct-home -provider-auth-home -model -debug -version; do rg -q -- "$$flag" docs/reference/cli.mdx cmd/acp-go-hermes/main.go || { echo "missing CLI flag in docs/code: $$flag"; exit 1; }; done
