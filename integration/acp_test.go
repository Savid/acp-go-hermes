...
func TestHermesACPAgentBinaryImportRestoreReplay(t *testing.T) {
	requireRunIntegration(t)
	...
	fixturePath := filepath.Join(t.TempDir(), "hermes-import.json")
	writeHermesImportFixture(t, fixturePath, nativeID, cwd)
	importCmd := exec.CommandContext(ctx, integrationHermesPath(t), "import", fixturePath, "--pure") // #nosec G204,G702 -- opt-in integration test command.
	importCmd.Dir = cwd
...
func nativeSessionIDFromMeta(t *testing.T, meta map[string]any) string {
	t.Helper()
	hermesMeta, _ := meta["hermes"].(map[string]any)
	nativeID, _ := hermesMeta["nativeSessionId"].(string)
	if nativeID == "" {
...
func writeHermesImportFixture(t *testing.T, path string, nativeID string, cwd string) {
	t.Helper()
